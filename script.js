...
const wrongQuestionsListElement = document.getElementById(
...
function generateQuestions(minRange, maxRange, skipNumbers) {
  ...
  if (possibleQuestions.length < questionCount) {
    alert(
      `Hanya tersedia ${possibleQuestions.length} soal dengan pengaturan saat ini.`
    );
...
function exitGame() {
  ...
  if (confirm('Yura yakin ingin keluar dari permainan?')) {
...
function handleCorrectAnswer() {
  ...
  // Update UI
  scoreElement.textContent = score;
  feedbackElement.textContent = `Benar! Yura Pintar! +${questionPoints} poin`;
...
function endGame() {
  ...
  if (percentage >= 90) {
    motivationMessage = 'Hebat sekali! Yura sangat pintar & jenius!';
  } else if (percentage >= 70) {
    motivationMessage = 'Bagus! Yura sudah menguasai perkalian dengan baik!';
  } else if (percentage >= 50) {
    motivationMessage = 'Lumayan! Teruslah berlatih perkalian!';
  } else {
    motivationMessage =
      'Jangan menyerah! Teruslah berlatih perkalian setiap hari!';
  }
  ...
  wrongQuestionsListElement.parentNode.insertBefore(
    motivationElement,
    wrongQuestionsListElement.nextSibling
  );
}
...
window.resolveLocalFileSystemURL(cordova.file.dataDirectory, function (dir) {
  ...
  // Bisa tambahkan logika untuk menyimpan file secara lokal di sini
});
